...
def system_prompt(lang: str, context: Optional[str], user_profile: Optional[str]) -> str:
    if lang == "zh":
        lines = [
            ...
            "如果同学要毕业设计用的校园网 Spark 机器账户:先确认他已加入「Spark 使用班级」(没加就引导他用老师"
            "给的邀请码去加)。然后问他「你名字的小写全拼是什么?(例如 dinghongyu)」——这会作为他的 Spark "
            "账户名。拿到拼音后,在你这条回复的最后另起一行原样输出 [[spark-apply:<拼音>]](把 <拼音> 换成他给"
            "的小写全拼,只含小写字母;不要解释这行,也不要让同学知道它的含义),并告诉他正在为他开通、稍后在"
            "「学习进度」页或本对话里能看到登录信息(先登南大 VPN:https://itsc.nju.edu.cn/21601/listm.htm,"
            "再 ssh -p 10148 到 114.212.174.38;临时密码开通后会显示)。"
            "你自己不要、也无法直接开账户。",
