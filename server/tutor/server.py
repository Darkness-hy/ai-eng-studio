...
def system_prompt(lang: str, context: Optional[str], user_profile: Optional[str]) -> str:
    if lang == "zh":
        lines = [
            ...
            "关于毕业设计用的校园网 Spark 机器账户:用户档案里的【Spark 账户信息】会告诉你他是否已加入"
            "「Spark 使用班级」、以及他的账户状态。处理「申请spark账号」时严格据此分支:"
            "① 已开通:直接告诉他登录方式(南大 VPN https://itsc.nju.edu.cn/21601/listm.htm,再 "
            "ssh -p 10148 <他的用户名>@114.212.174.38,临时密码在「学习进度」页),并提醒他首登必改密码——"
            "在「Current password」处把临时密码再原样输一遍(最好复制粘贴、别手敲,手敲输错会报 "
            "Authentication token manipulation error),再设自己的新密码;不要再让他申请,也不要输出任何标记;"
            "② 未加入「Spark 使用班级」:让他先用老师给的邀请码去「班级」页加入,加入后再来申请——此时不要发起申请、不要输出标记;"
            "③ 申请处理中:告诉他正在开通,请稍候或去「学习进度」页查看,不要重复发起;"
            "④ 上次开通失败:让他去「学习进度」页「撤回并重试」;"
            "⑤ 仅当『已入班 且 尚未申请』时:问他「你名字的小写全拼是什么?(例如 dinghongyu)」作为账户名,"
            "拿到后在回复最后另起一行原样输出 [[spark-apply:<拼音>]](只含小写字母;不要解释、不要让他知道这行的含义),"
            "并告诉他正在开通、稍后在「学习进度」页或本对话能看到登录信息。"
            "另外,凡是给同学 Spark 登录方式时,都要提醒他:Spark 是 ARM 架构(aarch64)、不是 x86,"
            "装依赖/编译/拉镜像时请选 ARM(aarch64)版。"
            "任何情况下你都不要、也无法自己直接开账户。",
