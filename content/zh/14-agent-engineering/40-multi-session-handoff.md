...
> 会话即将结束，但工作还没有结束。交接包（handoff packet）正是那个把「智能体干了一个小时」变成「下一个会话第一分钟就能高效推进」的产物。要有意识地构建它，而不是事后才想起来补。
...
- 识别每个交接包必须包含的七个字段。
- 从工作台产物自动生成交接，而不是手写文字。
- 把庞大的反馈日志裁剪成适合交接包的摘要。
- 让下一个会话的第一步行动具有确定性。

## 问题背景

会话结束了。智能体说「很好，我们取得了进展」。下一个会话打开，下一个智能体问「我们上次进行到哪儿了？」第一个智能体的回答已经不见了。于是下一个智能体重新摸索、重新运行同样的命令、向人类重新问同样的问题，花三十分钟才恢复到上一个会话最后三十秒的状态。
...
完整的 `feedback_record.jsonl` 可能有几百条记录。交接包只携带最后 K 条，再加上所有退出码非零的条目。下一个会话如有需要可以加载完整日志，但交接包本身保持小巧。
...
Codex CLI、Claude Code 和 OpenCode 各自实现了不同的压缩（compaction）方案；结构化交接包则架在这三者之上。

**压缩策略各不相同，交接包的模式不变。** Codex CLI 的 POST /v1/responses/compact 是服务端的不透明 AES blob（OpenAI 模型的快速路径）；回退方案是把本地的「交接摘要」以 `_summary` user 角色消息的形式追加进去。Claude Code 在上下文用量达到 95% 时执行五阶段渐进式压缩。OpenCode 采用基于时间戳的消息隐藏，外加一份五个标题的 LLM 摘要。三种机制不同，需求相同：把必须在压缩中存活的内容序列化为一个可移植的产物。交接包就是这个产物。
...
**在上下文用到 50-75% 之前收尾，而不是顶到上限。** 手写模式实践手册（CLAUDE.md + HANDOVER.md）报告称，会话在 50-75% 上下文预算时结束，效果优于撑到 95%。交接包生成器在压缩伪影污染源状态之前干净地运行。上下文完整时写交接很便宜；模型已经开始迷失方向时再写就很昂贵。
