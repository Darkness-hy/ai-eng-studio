...
> 2026 年的推理市场早已不是单纯的 GPU 时长租赁。它分化为三大阵营：定制芯片（Groq、Cerebras、SambaNova）、GPU 平台（Baseten、Together、Fireworks、Modal）和 API 优先的市场平台（Replicate、DeepInfra）。Fireworks 自 2026 年 5 月 1 日起将每块 GPU 的价格上调 $1/小时，其 $40 亿估值和每天 10 万亿以上 token 的处理量说明走量驱动的商业模式行得通。Baseten 于 2026 年 1 月以 $50 亿估值完成 $3 亿 Series E 融资。各家的竞争定位规则很简单：Fireworks 主打延迟、Together 主打模型目录广度、Baseten 主打企业级成熟度、Modal 主打 Python 原生开发体验、Replicate 主打多模态覆盖、Anyscale 主打分布式 Python。这节课会给你一张可以直接交给创始人的对比矩阵。
...
- 说出三大市场细分（定制芯片、GPU 平台、API 优先），并把每家厂商归入对应阵营。
- 解释为什么「按 token 计费」的 API 定价会向服务引擎的成本曲线压缩，而不是向硬件的成本曲线压缩。
- 计算至少三家厂商的单次请求有效成本，并解释什么时候按分钟计费（Baseten、Modal）优于按 token 计费。
- 针对给定的工作负载（serverless 突发型、稳定高吞吐、微调变体、多模态），判断哪个平台是合适的默认选项。

## 问题背景

你已经评估过托管的超大规模云平台，决定换一家更专注、更快的服务商——延迟选 Fireworks，模型广度选 Together，微调过的定制模型选 Baseten。现在摆在面前的是六个真实选项，而它们的定价页根本对不上号：Fireworks 标的是 $/百万 token；Baseten 标的是 $/分钟；Modal 标的是 $/秒；Replicate 标的是 $/次预测。不对工作负载建模，你根本没法把它们放在一起比较。

更麻烦的是，每张定价页背后的商业模式都不一样。Fireworks 在共享 GPU 上跑自家定制引擎（FireAttention），按 token 的费率反映的是他们的利用率曲线；Baseten 给你的是 Truss + 专属 GPU，按分钟计费反映的是独占性；Modal 是真正的 Python serverless——按秒计费，冷启动不到一秒。同样的产出（一条 LLM 响应），背后是三种不同的成本函数。

这节课会对这六家建模，并告诉你各自在什么场景下胜出。

## 核心概念

### 三大细分市场

**定制芯片** — Groq（LPU）、Cerebras（WSE）、SambaNova（RDU）。在同一模型上，解码速度通常比基于 GPU 的集群快 5-10 倍。按 token 的价格更高（2025 年末 Groq 在 Llama-70B 上约 $0.99/百万 token），但在延迟敏感的场景下无可匹敌。Groq 是语音智能体和实时翻译的生产首选。

**GPU 平台** — Baseten、Together、Fireworks、Modal、Anyscale。运行在 NVIDIA（2026 年的 H100、H200、B200）或偶尔 AMD 的硬件上。这是夹在「裸 GPU 租赁」（RunPod、Lambda）和「超大规模云托管服务」（Bedrock）之间的经济层。

**API 优先的市场平台** — Replicate、DeepInfra、OpenRouter、Fal。模型目录广，按次预测或按秒付费，强调「最快发出第一次调用」。

### Fireworks — 延迟优化型 GPU 平台

- FireAttention 引擎（自研）；宣传在同等配置下延迟比 vLLM 低 4 倍。
- 批处理档位（batch tier）约为 serverless 费率的 50%，面向非交互式工作负载。
- 微调模型按基础模型同等费率提供服务——相比那些对你的 LoRA 收溢价的服务商，这是实打实的差异化优势。
- 2026 年年中：自 2026 年 5 月 1 日起按需 GPU 租赁上调 $1/小时。规模化用量可议价。
- 财务信号：$40 亿估值，每天处理 10 万亿以上 token。

### Together — 广度优化型

- 200+ 模型，开源新模型发布后几天内即可上线。
- 在同等 LLM 模型上比 Replicate 便宜 50-70%——「AI Native Cloud」的定位靠的就是走量和目录广度。
- 推理 + 微调 + 训练统一在一个 API 里。

### Baseten — 企业级成熟度优化型

- Truss 框架：把依赖、密钥、服务配置打包进同一份模型清单。
- GPU 覆盖从 T4 到 B200。按分钟计费，冷启动缓解措施做得不错。
- SOC 2 Type II、HIPAA 就绪。金融科技和医疗行业的常见选择。
- $50 亿估值，2026 年 1 月 Series E（CapitalG、IVP、NVIDIA 投资 $3 亿）。

### Modal — Python 原生优化型

- 用纯 Python 写基础设施即代码。给函数加上 `@modal.function(gpu="A100")` 装饰器，一条命令即可部署。
- 按秒计费。预热后冷启动 2-4 秒；小模型不到 1 秒。
- $8700 万 Series B，估值 $11 亿（2025 年）。独立调研中开发者体验得分最高。

### Replicate — 多模态广度

- 按次预测付费。图像、视频和音频模型的默认平台。
- 集成生态丰富（Zapier、Vercel、CMS 插件）。
- LLM 按 token 费率竞争力较弱，但靠多模态品类多样性取胜。
...
按 token 计费适合对延迟不敏感、流量突发的工作负载——你只为实际用量付钱。按分钟计费适合利用率高且可预测的场景——一旦你把 GPU 打满，就能反超按 token 的价格。

经验法则：当工作负载对一块专属 GPU 的持续利用率超过约 30% 时，按分钟计费（Baseten、Modal）开始优于按 token 计费（Fireworks、Together）。低于这个水平则按 token 更划算，因为你不用为闲置时间买单。

### 真正的护城河是自研引擎

vLLM 和 SGLang 之上的每家平台都宣称有自研引擎：FireAttention、RayTurbo、Baseten 的推理栈。自研引擎的宣传带有营销色彩——诚实的说法是，vLLM + SGLang 占了生产环境开源推理的约 80%，平台层真正的差异化在于开发体验（DX）、归因和 SLA。

### 你应该记住的几个数字

- Fireworks GPU 租赁：自 2026 年 5 月 1 日起上调 $1/小时。
...
`code/main.py` 在一个合成工作负载上跨定价模型比较这六家厂商，报告 $/天和有效 $/百万 token。运行它，找出按 token 与按分钟计费之间的盈亏平衡点。
...
2. 你的产品同时提供图像生成、聊天和语音转文字。为每种模态选择平台，并说出能把它们统一起来的网关模式。
3. Fireworks 把你主力模型的价格上调 $1/小时。假设 40% 的流量迁移到批处理档位（五折），建模混合成本的影响。
...
| 批处理档位 | 「五折」 | 非交互式队列，费率打折；Fireworks、OpenAI 上常见 |
